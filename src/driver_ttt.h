...
/* core parameters */
#define MDRV_FRAMES_PER_SECOND(rate)				machine->frames_per_second = (rate);
#define MDRV_VBLANK_DURATION(duration)				machine->vblank_duration = (duration);
#define MDRV_INTERLEAVE(interleave) 				machine->cpu_slices_per_frame = (interleave);
#define MDRV_WATCHDOG_VBLANK_INIT(watch_count)		machine->watchdog_vblank_count = (watch_count);
#define MDRV_WATCHDOG_TIME_INIT(time)				machine->watchdog_time = (time);

/* core functions */
#define MDRV_MACHINE_INIT(name) 					machine->machine_init = machine_init_##name;
#define MDRV_MACHINE_STOP(name) 					machine->machine_stop = machine_stop_##name;
#define MDRV_NVRAM_HANDLER(name)					machine->nvram_handler = nvram_handler_##name;
...
#define MDRV_GFXDECODE(gfx) 						machine->gfxdecodeinfo = (gfx);
#define MDRV_PALETTE_LENGTH(length) 				machine->total_colors = (length);
#define MDRV_COLORTABLE_LENGTH(length)				machine->color_table_len = (length);


/* core video functions */
#define MDRV_PALETTE_INIT(name) machine->init_palette = palette_init_##name;
#define MDRV_VIDEO_START(name)	machine->video_start  = video_start_##name;
#define MDRV_VIDEO_STOP(name)	machine->video_stop   = video_stop_##name;
#define MDRV_VIDEO_EOF(name)	machine->video_eof	  = video_eof_##name;
//#define MDRV_VIDEO_EOF(name) ;

#define MDRV_VIDEO_UPDATE(name) 					machine->video_update = video_update_##name;


/* add/remove speakers */
#define MDRV_SPEAKER_ADD(tag, x, y, z)				machine_add_speaker(machine, (tag), (x), (y), (z));
#define MDRV_SPEAKER_REMOVE(tag)					machine_remove_speaker(machine, (tag));
#define MDRV_SPEAKER_STANDARD_MONO(tag) 			MDRV_SPEAKER_ADD(tag, 0.0, 0.0, 1.0)
...
#define MDRV_SOUND_ADD_TAG(tag, type, clock)		sound = machine_add_sound(machine, (tag), SOUND_##type, (clock));
#define MDRV_SOUND_ADD(type, clock) 				MDRV_SOUND_ADD_TAG(NULL, type, clock)
#define MDRV_SOUND_REMOVE(tag)						machine_remove_sound(machine, tag);
...
#define MAX_CPU 8		/*8 MAX_CPU is the maximum number of CPUs which cpuintrf.c */
						/* can run at the same time. Currently, 8 is enough. */

#define MAX_SOUND 32	/*32 MAX_SOUND is the maximum number of sound subsystems */
