		...
		else
		{
			...
			else
			{
				...
				case 0x03: /* tempo */
					...
					if(fa_tempo_cheat) pSequence->tempo *= 0.75;/*now fake div2 */
...
//
	...
	for( i=0;i<MAX_VOICE;i++ )
	{
		...
		if( (pVoice->bActive & 3) && pVoice->delta )
		{
			...
			INT16 lvol   = (pVoice->leftVolume )/2;
			INT16 rvol   = (pVoice->rightVolume)/2;
			...
			//	*pDest++ += dat*rvol/256;
				...
				dat = ReadPCMSample(chip,pos>>FIXED_POINT_SHIFT, pVoice->flags);
			//	*pDest++ += lvol*dat/8;
			//	*pDest++ += rvol*dat/8;
				(*pDest++) += lvol*dat;
				(*pDest++) += rvol*dat;
